              ...
              <tbody class="table-group-divider">
                <template v-for="(item, index) in items" :key="item.uuid">
                  <tr>
                    ...
                    <td>
                      <img :src="item.baked_img" class="img-fluid" :alt="item.name" />
                    </td>
                    <td>
                      {{ item.name }}<br />{{ item.ovr }}<br />{{ item.team }}<br />{{
                        item.display_position
                      }}
                    </td>
                    <td>{{ item.ability_name }}<br /><br />{{ item.ability_desc }}</td>
                    <template v-for="boost in item.boosts" :key="boost.tier">
                      <td>
                        <p class="card-text">
                          <small class="text-body-secondary"> {{ boost.description }}</small>
                        </p>
                        <p class="card-text">
...
const inquiryCaptains = async (mode) => {
  ...
  //API 호출
  const res = await fetch(`/api/captains?page=${page}`)
  //응답
  const data = await res.json()
  //현재 카운드
  current = page * data.per_page
  //전체 카운트
  total = data.total_captains
  //아이템 추가
  items.push(...data.captains)
  //더보기
