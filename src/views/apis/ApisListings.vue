    ...
    <section class="py-5 text-center container">
      <div class="row py-lg-5">
        <div class="col-lg-6 col-md-8 mx-auto">
          <h1 class="fw-light">MLB The Show 24 Captains</h1>
          <p class="lead text-body-secondary">...</p>
          <p>
            <a href="#" class="btn btn-primary my-2 me-2">Main call to action</a>
            <a href="#" class="btn btn-secondary my-2 ms-2">Secondary action</a>
          </p>
        </div>
      </div>
    </section>
          ...
          <div class="table-responsive">
            <table class="table table-hover">
              <thead class="table-dark">
                <tr>
                  <th scope="col">#</th>
                  <template v-for="(value, name) in header" :key="name">
                    <template v-if="!except.includes(name)">
              ...
              <tbody class="table-group-divider">
                <template v-for="(item, index) in items" :key="item.item.uuid">
                  <tr>
                    <td>{{ index + 1 }}</td>
                    <template v-for="(value, name) in item.item" :key="name">
                      <template v-if="!except.includes(name)">
                        <template v-if="name.includes('img')">
                          <td><img :src="value" class="img-fluid" style="height: 80px" /></td>
                        </template>
                        <template v-else>
                          <td>{{ value }}</td>
                        </template>
...
const inquiryListings = async (mode) => {
  ...
  //API 호출
  const res = await fetch(`/api/listings?type=mlb_card&series=1337&page=${page}`)
  //응답
  const data = await res.json()
  //현재 카운드
  current = page * data.per_page
  //아이템 추가
  items.push(...data.listings)
  //헤더 추가
  header.value = Object.assign({}, items.at(0).item)
  //더보기
